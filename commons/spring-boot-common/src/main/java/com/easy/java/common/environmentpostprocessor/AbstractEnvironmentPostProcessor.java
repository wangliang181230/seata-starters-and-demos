...
public abstract class AbstractEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
	...
	public static MapPropertySource getDefaultPropertySource(ConfigurableEnvironment environment, boolean autoCreate) {
		...
		if (propertySource != null && propertySource instanceof MapPropertySource) {
			...
			if (propertySource != null) {
				newPropertySource = new MapPropertySource(DEFAULT_PROPERTIES + "2", new HashMap<String, Object>());
				propertySources.addBefore(DEFAULT_PROPERTIES, newPropertySource);
			} else {
				newPropertySource = new MapPropertySource(DEFAULT_PROPERTIES, new HashMap<String, Object>());
	 ...
	 */
	protected PropertySource parseConfigFile(String propertySourceName, String profilePath) {
		try {
			// 创建资源对象
	...
	protected void loadConfigFile(ConfigurableEnvironment environment, String propertySourceName, String profilePath) {
		// 解析配置文件，获取配置源
		PropertySource propertySource = this.parseConfigFile(propertySourceName, profilePath);
